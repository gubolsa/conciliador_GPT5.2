...
if fatura_file.name.lower().endswith(".pdf"):
    ...
    with c1:
        col_data_f = st.selectbox("Coluna de data", cols_f, index=0)
    with c2:
        col_desc_f = st.selectbox("Coluna de descrição", cols_f, index=1 if len(cols_f)>1 else 0)
    with c3:
        col_val_f = st.selectbox("Coluna de valor", cols_f, index=3 if len(cols_f)>3 else min(2, len(cols_f)-1))
...
c1, c2, c3 = st.columns(3)
with c1:
    col_data_m = st.selectbox("Coluna de data (Mobills)", cols_m, index=0)
with c2:
    col_desc_m = st.selectbox("Coluna de descrição (Mobills)", cols_m, index=min(1, len(cols_m)-1))
with c3:
    col_val_m = st.selectbox("Coluna de valor (Mobills)", cols_m, index=min(2, len(cols_m)-1))
